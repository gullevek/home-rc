...
precmd () { __git_ps1 "[%F{yellow}%*%f][%F{green}%n%f@%F{red}%m%f:%F{blue}%~%f][%F{cyan}%j%f]" "[%(?.%F{green}√.%F{red}?%?)%f]%(!.#.$) " "[%s]" }
# DIRECT PROMPT
#setopt PROMPT_SUBST ; PS1='[%{$fg[yellow]%}%*%{$reset_color%}][%{$fg[green]%}%n%{$reset_color%}@%{$fg[red]%}%m%{$reset_color%}%{$reset_color%}:%{$fg[blue]%}%~%{$reset_color%}]$(__git_ps1 "[%s]")\$ '
# PROMPT PLAIN NO GIT (long color)
#PS1="[%{$fg[yellow]%}%*%{$reset_color%}][%{$fg[green]%}%n%{$reset_color%}@%{$fg[red]%}%m%{$reset_color%}:%{$fg[blue]%}%~%{$reset_color%}]\$ "
# RIGHT PROMPT
#RPROMPT="[%{$fg[yellow]%}%D %D{%H:%M:%S.%.}%{$reset_color%}]"
RPROMPT="[%{$fg[yellow]%}%D%{$reset_color%}]"

# GIT STUFF START [RIGHT SIDE]
